...
2. 한 문장으로 제품 제한을 알린다: “이 브라우저에 익명으로 저장되며 계정 복구와 기기 동기화는 제공하지 않아요.”
...
- `Workspace`: 익명 사용자 경계, IANA timezone, session token hash, 생성/갱신 시각
...
| --- | --- | --- | --- |
| `POST /session` | `{ timezone }` | 201, private cookie와 `{ csrfToken }`; 기존 유효 세션이면 200 재사용 | 400 `INVALID_TIMEZONE` |
| `GET /bootstrap` | 없음 | 200 `{ workspace, activeQuest, currentAction, pendingAdaptation, recentAttempts, nextRequiredAction, csrfToken }` | 401 `SESSION_REQUIRED` |
...
`POST /session`은 아직 session/CSRF가 없으므로 허용된 `Origin`만 엄격히 검사한다. 그 외 모든 쓰기는 session cookie, `X-CSRF-Token`, `Origin` 검사를 요구한다. 자원 생성/결과 기록 endpoint는 UUID 형식의 `Idempotency-Key`도 요구한다. 클라이언트가 `workspaceId`를 보내거나 URL에 넣는 API는 만들지 않는다.
...
- 운영 cookie는 `HttpOnly`, `Secure`, `SameSite=Lax`, 제한된 path와 만료를 사용한다.
...
- mock HTTP 테스트로 성공, validation, 401 session 재생성, 409 refetch, 네트워크 재시도를 검증한다.
