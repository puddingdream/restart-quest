...
- workspace 삭제 204 뒤에는 이전 controller 스냅샷과 브라우저 저장 상태를 폐기하고 삭제 완료 화면을 구성한다. 삭제 요청의 성공 본문이나 `nextRequiredAction`을 가정하지 않으며, 사용자가 명시적으로 새 시작을 선택한 뒤에만 새 session/bootstrap을 만든다.
...
- 성공 write 뒤 bootstrap 재검증과 삭제 뒤 명시적 새 session/bootstrap 경계를 지켜 stale 자원 스냅샷을 렌더링하지 않는다.
...
- backend/frontend production build, PostgreSQL, same-origin proxy와 UTC 03:00 cleanup scheduler를 `compose.yaml`로 연결한다.
...
- release package가 설정할 환경 변수 이름은 `DB_URL`, `DB_USERNAME`, `DB_PASSWORD`, `RQ_ALLOWED_ORIGINS`, `RQ_COOKIE_SECURE`로 고정한다. 저장소와 `compose.yaml`에 운영 secret 값이나 안전하지 않은 production 기본값을 넣지 않는다.
