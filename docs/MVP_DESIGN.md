...
| `release-stack` | `backend-infra` | 두 Dockerfile/.dockerignore, `frontend/nginx.conf`, `compose.yml`, `scripts/verify-release.ps1`, `scripts/verify-release.sh`, `docs/RUNBOOK.md` | compose config/build/up/health/same-origin smoke, shell syntax, 비파괴 rollback 검토 |
| `release-e2e` | `frontend` | 독립 `e2e/` workspace와 E2E 결과 ignore를 위한 루트 `.gitignore` | Playwright 핵심 흐름, reload, 두 세션, stale/중복, timeout/proxy, responsive/a11y 결과 |
