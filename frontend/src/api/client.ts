...
export class QuestApiClient implements QuestApi {
  ...
  async createJourney(input: CreateJourneyInput, commandId: string): Promise<JourneySnapshot> {
    const body = {
      ...createJourneyInputSchema.parse(input),
      commandId: uuidSchema.parse(commandId),
    }
  ...
  async completeQuest(
    ...
    return (
      await this.request({
        method: 'POST',
        path: `/api/v1/quests/${encodeURIComponent(uuidSchema.parse(questId))}/complete`,
        successStatuses: [200],
  ...
  async reframeQuest(
    ...
    return (
      await this.request({
        method: 'POST',
        path: `/api/v1/quests/${encodeURIComponent(uuidSchema.parse(questId))}/reframe`,
        successStatuses: [200],
        schema: transitionResultSchema,
        body,
      })
  ...
  private async request<T>(options: RequestOptions<T>): Promise<{ data: T; status: number }> {
    ...
    try {
      ...
      if (cause instanceof QuestApiError || cause instanceof ApiContractError) {
        throw cause
      }
      throw new QuestNetworkError(didTimeout ? 'timeout' : 'network', { cause })
    } finally {
  ...
  private async readJson(response: Response): Promise<unknown> {
    try {
      ...
      throw new ApiContractError(`HTTP ${response.status} 응답을 JSON으로 해석할 수 없습니다.`, {
        cause,
      })
    }
  }
